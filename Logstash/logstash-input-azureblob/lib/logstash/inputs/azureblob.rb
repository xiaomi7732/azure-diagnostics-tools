...
class LogStash::Inputs::RegistryItem
  attr_accessor :file_path, :etag, :offset, :reader
  ...
  def initialize(file_path, etag, reader, offset = 0)
      @file_path = file_path
      @etag = etag
      @reader = reader
      @offset = offset
...
class LogStash::Inputs::LogstashInputAzureblob < LogStash::Inputs::Base
  ...
  def process(queue)
    begin
      blob_items = list_all_blobs
      blob, start_index = register_for_read
      ...
      if(!blob.nil?)
        begin
          ...
          new_registry_item = LogStash::Inputs::RegistryItem.new(blob_name, new_etag, nil, new_offset)
  ...
  def deserialize_registry_hash (json_string)
    ...
    temp_hash.values.each {
      |kvp| result[kvp["file_path"]] = LogStash::Inputs::RegistryItem.new(kvp["file_path"], kvp["etag"], kvp["reader"], kvp["offset"])
    }
    return result
  ...
  def register_for_read
    begin
      ...
      candidate_blobs.each { |candidate_blob|
        ...
        registry_item = LogStash::Inputs::RegistryItem.new(candidate_blob.name, candidate_blob.properties[:etag], nil, 0) if registry_item.nil?
        
        if ((registry_item.offset < candidate_blob.properties[:content_length]) && (registry_item.reader.nil? || registry_item.reader == @reader))
          picked_blob = candidate_blob
      ...
      return picked_blob, start_index
    rescue StandardError => e
  ...
  # Load the content of the registry into the registry has and return it.
  def load_registry
    # Get content
